...
class BattingSpider(scrapy.Spider):
    name = "batting"
    start_urls = ["https://indianexpress.com/section/sports/ipl/"]

    def parse(self, response):
        attr=response.css("#iplStats-1 .constituency-list__th .table-data::text").getall()
        attr.insert(0,"NAME")

        for i in response.css("#iplStats-1 .constituency-list__tr"):
